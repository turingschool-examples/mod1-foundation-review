...
describe('iterator', function() {
  ...
  it('can create a string of YELLING UPPER CASE words using FOR EACH', function() {
    var factory = new Iterator();
  ...
  it('using FOR EACH can sum a bunch of numbers', function() {
    var factory = new Iterator();
  ...
  it('using FOR EACH can square every number in a collection and then sum them', function() {
    var factory = new Iterator();
  ...
  it.skip('can FILTER out strings shorter or equal to 4 letters', function() {
    ...
    assert.deepEqual(factory.shortWords(['potato', 'bird', 'ham', 'lasagna', 'pants', 'bro', 'consternation']), ['bird', 'ham', 'bro']);
  });

  it.skip('can return a new array of uppercase strings using MAP', function() {
    var factory = new Iterator();

    assert.deepEqual(factory.uppercaseArray(['potato', 'bird', 'ham', 'pants']), ['POTATO', 'BIRD', 'HAM', 'PANTS']);
  });

  it.skip('can return a new array of integers that are increased by 10 using MAP', function() {
    var factory = new Iterator();

    assert.deepEqual(factory.addTen([1, 2, 3, 4, 5]), [11, 12, 13, 14, 15]);
  });
  ...
  it.skip('can SORT an array of numbers', function() {
    ...
    assert.deepEqual(factory.sorted([5, 2, 6, 3, 1]), [1, 2, 3, 5, 6]);
    assert.deepEqual(factory.sorted([8, 10, 12, 7, 3]), [3, 7, 8, 10, 12]);
  });

  it.skip('can SORT an array of numbers in reverse order', function() {
    var factory = new Iterator();

    assert.deepEqual(factory.sorted2([5, 2, 6, 3, 1]), [6, 5, 3, 2, 1]);
    assert.deepEqual(factory.sorted2([8, 10, 12, 7, 3]), [12, 10, 8, 7, 3]);
  });

  it.skip('can SORT an array of strings', function() {
    var factory = new Iterator();

    assert.deepEqual(factory.sorted(['zoo', 'apple', 'zucchini', 'potato', 'beets']), ['apple', 'beets', 'potato', 'zoo', 'zucchini']);
  });
  ...
  it.skip('can MAP over a SORTED array returning meals in order of most ingredients ', function() {
    ...
    assert.deepEqual(factory.sortByIngredients(meals), expected);
  });
